...
def f_oper(x,t,*coef_tup): # eco operator returns the equations
  x1=x[0]
  y1=x[1]
  x2=x[2]
  y2=x[3]
  ...
  if coef_tup[7]=='DC':
    f_op = [y1,
         (-k1 * x1/m1 - zeta1 * y1/m1 + (fe/m1) / (s0 - x1 - x2)**2.),
         y2,
         (-k2 * x2/m2 - zeta2 * y2/m2+ (fe/m2)/(s0 - x1 - x2)**2.)]
  else:
    f_ac = coef_tup[7]
    f_op = [y1,
         (-k1 * x1/m1 - zeta1 * y1/m1 + (fe*np.sin(2*np.pi*eval(f_ac)*t)/m1) / (s0 - x1 - x2)**2.),
         y2,
         (-k2 * x2/m2 - zeta2 * y2/m2+ (fe*np.sin(2*np.pi*eval(f_ac)*t)/m2)/(s0 - x1 - x2)**2.)]
  return f_op #need to keep coeff in () ?

"""
def f_oper_AC(x,t,*coef_tup): # eco operator returns the equations
  x1=x[0]
  y1=x[1]
  x2=x[2]
  y2=x[3]
...
def sol(t,coef,x0,tol): #numerical solution of the model
  ys = spi.odeint(f_oper, x0, t, args=tuple(coef), atol=tol[0], rtol=tol[1])
  #print (ys[:,2])
  return (ys[:,0],ys[:,1],ys[:,2],ys[:,3])
  
def fitsol(t,thcoef,fitcoef,x0,tol,numdrops):
  coef=[None]*8
  coef[0] = thcoef[0]
...
def residual(x_model,import_data):
...
def fitalgo(imgdata,thcoef,x0,tol,numdrops): #Global optimization to give fitted coefficients of eco model; del_m,s0 are fixed
  tdata=imgdata.tdata
...
def fit(optiter,imgdata,thcoef,x0,tol,numdrops):
...
class manual_model(object):
      ...
      def __init__(self,coef,t,x0):
        ...
        self.x_manual= sol(self.t,self.coef,self.x0,self.tol) #manual solution
...
def fit_env_min_iter(optiter,envdata_t,envdata_x):
  resid=[]
  fit_iter=[]
  for i in range(1,optiter+1):
...
def fit_env_max_iter(optiter,envdata_t,envdata_x):
  resid=[]
  fit_iter=[]
  for i in range(1,optiter+1):
...
def fitenv_min(envdata_t,envdata_x):
...
def fitenv_max(envdata_t,envdata_x):
  #t_data1 =(envdata_t[0],envdata_t[len(envdata_t)-1],(envdata_t[1]-envdata_t[0])/4)
...
def envfit_to_guesscoeff(par,envfit,numdrops):
...
def finddynphase(imagedata,envdata,imgenvfit,coef): # file loop above this call and plt.show() after the loop ends
